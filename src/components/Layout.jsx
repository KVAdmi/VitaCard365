...
const Layout = ({ children, title, showBackButton = false }) => {
  ...
  // Detección de plataforma nativa (igual que en src/lib/auth.ts)
  ...
  return (
    <div className="min-h-[100dvh] bg-vita-blue text-vita-white">
      {!crystalActive && (
      <div className={isNative ? 'safe-area-top' : ''}>
  <div className="glass-card rounded-3xl shadow-2xl mx-2 mt-6 mb-4 px-4 py-3 flex items-center justify-between">
  ...
  <nav className="fixed bottom-0 left-0 right-0 z-40 glass-card footer-menu app-tabbar-safe">
    <div className="flex justify-around py-2 items-center">
      ...
      {/* Botón logout como ícono de puerta, solo si hay usuario */}
      {user && (
        <button
          onClick={logout}
          title="Cerrar sesión"
          aria-label="Cerrar sesión"
          className="ml-2 p-0 bg-transparent border-none outline-none focus:ring-2 focus:ring-vita-orange/70 hover:opacity-80 transition"
          style={{ background: 'none', boxShadow: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="none" viewBox="0 0 24 24" style={{ color: '#FFB300', display: 'block' }}>
            <rect x="5" y="3" width="10" height="18" rx="2" stroke="#FFB300" strokeWidth="2"/>
            <path d="M15 12h4m0 0-2-2m2 2-2 2" stroke="#FFB300" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            <circle cx="9.5" cy="12" r="1" fill="#FFB300" />
          </svg>
        </button>
      )}
    </div>
