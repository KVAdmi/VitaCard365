        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center bg-black/30 backdrop-blur-sm">
          <div className="relative w-full max-w-full h-full m-0 glass-card rounded-none shadow-xl border border-white/20 flex flex-col justify-center items-center">
            ...
            <div className="p-6 w-full max-w-2xl mx-auto">
              ...
              {/* Chat UI funcional */}
              <div className="h-[60vh] bg-white/10 rounded-lg p-3 mb-3 overflow-y-auto border border-white/10 flex flex-col gap-2 text-base">
                {chatMessages.map((msg, i) => (
                  <div key={i} className={`text-sm ${msg.from === "bot" ? "text-blue-400 text-left" : "text-white text-right"}`}>
                    {msg.text}
                  </div>
                ))}
                {loadingChat && (
                     ...
                     <button
                       ...
                       onClick={async () => {
                         try {
                           ...
                           // Aquí puedes iniciar la grabación o reconocimiento de voz
                         } catch (err) {
